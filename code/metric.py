...
def get_dice_1(true, pred):
    ...
    if np.sum(denom)==0:
        ...
        dice = 2.0 * np.sum(inter) / np.sum(denom)
    return dice
...
###############################################################################################################
def get_fast_aji_plus_stacked(true, pred):
    """
    AJI+, an AJI version with maximal unique pairing to obtain overall intersecion.
    Every prediction instance is paired with at most 1 GT instance (1 to 1) mapping, unlike AJI
    where a prediction instance can be paired against many GT instances (1 to many).
    ...
    true = np.copy(true)  # ? do we need this
    pred = np.copy(pred)
    shape_true = true.shape
    num_cells = shape_true[2]
    #true_id_list = list(np.unique(true))
    pred_id_list = list(np.unique(pred))
    ...
    # prefill with value
    pairwise_inter = np.zeros([num_cells,
                               len(pred_id_list) - 1], dtype=np.float64)
    pairwise_union = np.zeros([num_cells,
                               len(pred_id_list) - 1], dtype=np.float64)

    # caching pairwise
    for true_id in range(num_cells):#true_id_list[1:]:  # 0-th is background
        #t_mask = true_masks[true_id]
        t_mask = true[:,:, true_id]
    ...
    overall_union = paired_union.sum()
    # add all unpaired GT and Prediction into the union
    unpaired_true = np.array([idx for idx in range(num_cells) if idx not in paired_true])
    unpaired_pred = np.array([idx for idx in pred_id_list[1:] if idx not in paired_pred])
    for true_id in unpaired_true:
        overall_union += true[:,:, true_id].sum()
    for pred_id in unpaired_pred:
        overall_union += pred_masks[pred_id].sum()
    #
    aji_score = overall_inter / overall_union
